   ...
   $.fn.extend({
        MSDL: function (options){/*MultiSelectDropList*/
		 ...
		 //插件实现代码
			...
			$ipt.width(options.width - 1);//设定文本框宽度
			...
			//2.创建 全选和确认按钮  container_top层 
			var $container_top = $('<div class="container_top"></div>');//外层div包裹
            ...
            $all.appendTo($container_top);
            $btn.appendTo($container_top);
			...
			for(var i = count-1; i >= 0; i--){
			   ...
			   var $list = $('<div><input type="checkbox" value='+options.data[i]+' />'+options.data[i]+'<br></div>');
			   $list.appendTo($content);
			}
           
			//4把container_top层和content层加到$container下
			$container_top.appendTo($container);
